...
bool uart_init(const char* dev = "/dev/ttyS0", int baud = 115200)
{
    uart_fd = open(dev, O_RDWR | O_NOCTTY | O_NONBLOCK);
    if (uart_fd < 0) {
        std::cerr << "[UART] Không mở được " << dev << "\n";
        return false;
    }
    ...
    std::cout << "[UART] OK: " << dev << "\n";
    return true;
}

void uart_send(const std::vector<Detection>& dets)
{
    if (uart_fd < 0) return;
    ...
    if (dets.empty()) {
        n = snprintf(buf, sizeof(buf), "NONE\n");
    } else {
        const Detection& best = *std::max_element(dets.begin(), dets.end(),
        ...
        if (best.class_id == 0) {
            ...
            n = snprintf(buf, sizeof(buf), "%s\n",
                CLASS_NAMES[best.class_id].c_str());
        }
    }
...
if (d.class_id > 0 && d.class_id < CLASS_NAMES.size()) {
...
for (auto& d : last_dets) {
    ...
    if (!is_valid_color(frame, d, CLASS_NAMES[d.class_id])) {
        // ❌ bỏ luôn SP_loi (KHÔNG add vào filtered)
        continue;
    }
...
// 👉 dùng filtered thay vì last_dets
last_dets = filtered;
